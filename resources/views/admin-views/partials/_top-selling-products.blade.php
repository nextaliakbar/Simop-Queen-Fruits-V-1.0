<div class="card-header d-flex justify-content-between gap-10">
    ...
    <a href="{{route('admin.product.list')}}" class="btn-link">Tampilkan Semua</a>
</div>
        ...
        @foreach($top_sell as $key=>$item)
            @if(isset($item->product))
                <a class="d-flex justify-content-between align-items-center text-dark" href="{{route('admin.product.view',[$item['product_id']])}}">
                    ...
                    <span class="px-2 py-1 badge-soft-c1 font-weight-bold fz-12 rounded lh-1">Terjual : {{$item['count']}}</span>
                </a>
            @endif
        @endforeach
    </div>
